(function () {
  ...
  function createAutoCompleteList(movies) {
    // Create autocomplete suggestions and add to DOM
    let html = '';

    if (!movies || movies.length === 0) {
      html += createAutocompleteItemNotFound();
    } else {
      movies.forEach(movie => html += createAutocompleteItem(movie));
    }
    autoCompleteList.innerHTML = html;
    autoCompleteList.hidden = false;
  }

  function createAutocompleteItemNotFound(item) {
    return `<li class="autocomplete__item autocomplete__item--notfound">No results found</li>`;
  }

  function createAutocompleteItem(item) {
    return `<li class="autocomplete__item" id="${item.id}">${item.title}</li>`;
  }
  ...
  function setActiveListItem() {
    ...
    // If current item is outside of visible part of autocomplete list, scroll to it's position
    const currentElHeight = listItems[currentAutoCompleteIndex].offsetHeight;
    const currentElOffset = currentElHeight * currentAutoCompleteIndex;
    const viewport = autoCompleteList.scrollTop + autoCompleteList.offsetHeight;

    if (currentElOffset < autoCompleteList.scrollTop || (currentElOffset + currentElHeight) > viewport) {
      autoCompleteList.scrollTop = currentElOffset;
    }

    listItems[currentAutoCompleteIndex].classList.add("autocomplete__item--is-active");
  }

  function deactivateAutocompleteItems(x) {
    const listItems = autoCompleteList.querySelectorAll('.autocomplete__item');
  ...
  async function fetchMovies(query) {
    try {
      const res = await fetch(`https://api.themoviedb.org/3/search/movie?api_key=${API_KEY}&language=en-US&query=${sanitizeHTML(query)}&page=1&include_adult=false`);
      const data = await res.json();

      return data.results ? data.results.map(movie => ({ id: movie.id, title: movie.title, poster: movie.poster_path })) : null;
    } catch (err) {
      throw err;
    }
  }
  ...
  function createSearchHistoryItem(item) {
    return `<li class="search-history__item" id="${item.id}">${item.query} 
      ...
      <button class="search-history__delete-item" aria-label="Delete">
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    // Autocomplete on every new input
    input.addEventListener('input', async (event) => {
      if (!input.value) {
        emptyAutocompleteList();
      } else {
        const movies = await fetchMovies(input.value);
        createAutoCompleteList(movies);
      }
    ...
    // Add click event listener on Auto complete Items 
    autoCompleteList.addEventListener('click', event => {
      if (!event.target.matches('.autocomplete__item')) return;
      autoCompleteInput(event.target.textContent);
    }, false);

    // Add click event listener on Search History Items delete button
    searchHistoryList.addEventListener('click', event => {
      if (!event.target.matches('.search-history__delete-item') || event.target.matches('.autocomplete__item--notfound')) return;
      removeItemFromSearchHistory(event.target.parentNode.id);
    });
  });
})()